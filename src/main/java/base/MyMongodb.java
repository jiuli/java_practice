...
public class MyMongodb{
	...
	private final static MyMongodb UniqueInstance = new MyMongodb();
	
	public static MyMongodb getMongoInstance(){
	...
	private MyMongodb(String host,Integer port){
		this.host = host;
		this.port = port;
		_init();
	}
...
//			this.mg = new MongoClient(this.address,Arrays.asList(this.credential));
	...
	public static void main(String[] args){
		MyMongodb  mm= MyMongodb.getMongoInstance(); 
		DBCollection projects = mm.getTable("project");
		DBCollection qs = mm.getTable("question");
		DBCursor p_c = projects.find(new BasicDBObject("user_id",314610098));
		while(p_c.hasNext()){
